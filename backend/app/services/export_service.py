...
def build_excel(
    ...
    if pool_config:
        ...
        ws_params.append(["group_combine_mode", getattr(pool_config, "group_combine_mode", "best")])
        ws_params.append(["knockout_combine_mode", getattr(pool_config, "knockout_combine_mode", "best")])
        if pool_config.group_cap is not None:
            ws_params.append(["group_cap", float(pool_config.group_cap)])
        if pool_config.knockout_cap is not None:
            ws_params.append(["knockout_cap", float(pool_config.knockout_cap)])
        ws_params.append(["knockout_scoring_basis", getattr(pool_config, "knockout_scoring_basis", "ninety_minutes")])
